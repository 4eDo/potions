...
function formatParts(potion) {
    if (!potion.parts?.length) return "";
    return `<h4>Используеме части:</h4><p>${potion.parts.join(', ')}</p>`;
}
...
function formatComposition(potion) {
    if (!potion.composition?.length) return "";
    return `<h4>Химический состав:</h4><p>${potion.composition.join(', ')}</p>`;
}
...
function formatNeeded(potion) {
    ...
    return `<h4>Состав:</h4><p>${items.join(', ')}</p>`;
}
